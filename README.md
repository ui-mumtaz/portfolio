# Mumtaz Ahmad — Portfolio

**Senior Angular Developer | Front-End Architect | 15+ Years**

🚀 **Live:** [ui-mumtaz.github.io/portfolio](https://ui-mumtaz.github.io/portfolio/)

---

## About

Interactive portfolio showcasing 15+ years of experience in enterprise-grade Angular applications, full-stack JavaScript development, and frontend architecture.

## Tech Expertise

- **Frontend:** Angular (v2–v21), TypeScript, RxJS, NgRx, Signals, Standalone Components
- **Backend:** Node.js, Express.js, MongoDB, REST APIs
- **Architecture:** Nx Monorepo, Micro-Frontends (Module Federation, Single-SPA)
- **Design Systems:** Storybook, Angular Material, PrimeNG, WCAG Accessibility
- **Real-Time:** WebSockets, STOMP Protocol
- **Cloud:** AWS (S3, EC2, Lambda, CloudFront)

## Domains

Retail & E-commerce · Gaming & Entertainment · Telecom & Enterprise

## Connect

- 📧 Email: mumtazahmad2004@gmail.com
- 💼 LinkedIn: [linkedin.com/in/mumtazahmad](https://linkedin.com/in/mumtazahmad)
- 🟢 Upwork: [upwork.com/freelancers/buildwithangular](https://upwork.com/freelancers/buildwithangular)
- 🌐 Portfolio: [ui-mumtaz.github.io/portfolio](https://ui-mumtaz.github.io/portfolio/)

---

> Open to remote, hybrid, or on-site opportunities worldwide including Gulf/GCC.
